        ...
        {activeCd !== null && currentCd && (
          ...
          >
            ...
            >
              ...
              {/* LEFT WING: Cover Photo + Google Drive iframe player */}
              <div className="w-full md:w-1/2 h-full flex flex-col justify-between p-6 relative overflow-hidden rounded-t-2xl md:rounded-l-2xl md:rounded-tr-none bg-zinc-950 select-none">
                ...
                {/* Google Drive iframe embed — this is the actual audio player */}
                {isValidDriveId(currentDriveId) && (
                  <iframe
                    key={`${currentDriveId}-${activeSongIndex}`}
                    src={driveIframeSrc(currentDriveId)}
                    allow="autoplay"
                    className="absolute bottom-16 left-0 right-0 w-full z-30 opacity-0 pointer-events-none"
                    style={{ height: "80px" }}
                    title="audio-player"
                  />
                )}

                {/* Tap-to-show player overlay */}
                <div
                  onClick={() => setIsPlaying(p => !p)}
                  className="relative z-20 flex-1 flex flex-col items-center justify-center cursor-pointer group"
                >
                  <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/20 transition-colors">
                    <div className={`w-16 h-16 rounded-full bg-black/50 backdrop-blur-xs border border-white/20 flex items-center justify-center text-white shadow-2xl transition-all ${isPlaying ? 'opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100' : 'opacity-90 scale-100'}`}>
                      {isPlaying ? (
                        <svg className="w-6 h-6 fill-white" viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
                      ) : (
                        <svg className="w-6 h-6 fill-white translate-x-[2px]" viewBox="0 0 24 24"><polygon points="5,3 19,12 5,21"/></svg>
                      )}
                    </div>
                  </div>
                ...
                {!isValidDriveId(currentDriveId) && (
                  <div className="absolute inset-0 flex items-center justify-center z-25 pointer-events-none">
                    <div className="bg-black/50 backdrop-blur-sm rounded-xl px-4 py-3 text-center border border-white/10">
                      <Music className="w-6 h-6 text-purple-300 mx-auto mb-1 opacity-60" />
                      <p className="text-[10px] text-white/50 font-serif italic">No audio linked</p>
                    </div>
                  </div>
                )}
                    ...
                    {isPlaying && Array.from({ length: 5 }).map((_, i) => {
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{ opacity: [0, 0.75, 0], y: 40, x: 40 + i * 45 + Math.sin(i) * 20, scale: [0.6, 1.1, 0.7] }}
                    ...
                    {Array.from({ length: 28 }).map((_, i) => {
                      ...
                      return (
                        <motion.div
                          ...
                          animate={isPlaying ? { height: [4, 24, 6, 28, 4] } : { height: 4 }}
              ...
              <div className="w-full md:w-1/2 h-full bg-[#fcfaf4] relative flex flex-col overflow-y-auto rounded-b-2xl md:rounded-r-2xl md:rounded-bl-none">
                ...
                {/* Google Drive iframe — visible player for interaction */}
                {isValidDriveId(currentDriveId) && (
                  <div className="w-full px-4 pt-3 pb-1 shrink-0 z-20 relative">
                    <iframe
                      key={`visible-${currentDriveId}-${activeSongIndex}`}
                      src={driveIframeSrc(currentDriveId)}
                      allow="autoplay"
                ...
                <div className="flex-1 min-h-0 relative z-0 flex flex-col justify-center items-center">
                  {activeCd === 0 ? (
                    <div className="w-full h-full flex flex-col justify-center items-center"><PlaylistCover /></div>
                  ) : activeCd === 54 ? (
                    <div className="w-full h-full flex flex-col justify-center items-center"><PlaylistEnd /></div>
                  ) : (
                    <div className="w-full h-full flex flex-col justify-center items-center"><ScrapbookJournalPage pageNumber={activeCd} /></div>
                  )}
                </div>
              </div>
